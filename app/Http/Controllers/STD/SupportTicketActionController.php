...
use App\Employee;
use App\Models\STD\SupportTicketAssignHistory;
use Carbon\Carbon;
...
use App\Mail\StudentSupportPermissionMail;
use App\Mail\StudentSupportTicketNotificationMail;
...
class SupportTicketActionController extends Controller
{
    public function index(Request $request)
    {
        ...
        ])->where('status', $request->status)->paginate(50);
    ...
    public function show(Request $request, $id)
    {
        ...
        /*SupportTicketReply::where([
            ...
            'status' => 'unseen',
        ])->whereNull('created_by')->update(['status' => 'seen']);*/


        $studentAllSupportTickets = SupportTicketResource::collection(SupportTicket::whereCreatedBy($supportTicket->user->ID)->orderBy('id', 'desc')->get());

        $data = [
            'supportTicket' => $supportTicket,
            'studentAllSupportTickets' => $studentAllSupportTickets
        ];
    ...
    public function supportTicketStatus(Request $request)
    {
        ...
        $message = "Support Ticket {$request->status} successfully";
    ...
    public function studentAssignShow(Request $request, $id)
    {
        $supportTicket = new SupportTicketResource(SupportTicket::with(
            'user',
            'deny',
    ...
    public function requestForPermission(Request $request)
    {
        try {
            ...
            foreach ($emailArray as $email) {
                ...
                Mail::to($email)->send(
                    new StudentSupportPermissionMail($data, $supportTicket, $employee)
                );
            }

            //Send mail confirmation code..
            Mail::to($supportTicket->user->EMAIL)->send(
                new StudentSupportTicketNotificationMail($supportTicket, $employee)
            );
