...
class AdmisisonFormController extends Controller
{
    ...
    public function search(Request $request, $form)
    {
        ...
        {
            $form_info = FormDetails::with('user:name,id')->where('form_number', $form)->first();

            $form_info['sale_date'] = Carbon::parse($form_info->sale_date)->format('d M, Y');

            
            return response($form_info, 302);
        }
    ...
    public function english_book_search(Request $request, $form)
    {
        ...
        if(empty($form_details))
        {
            $form_info = EnglishBookFormDetails::with('user:name,id')->where('form_number', $form)->first();
