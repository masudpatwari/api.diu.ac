...
<p>This is to certify that {{ $profile['name'] }}, Passport No - {{ $profile['passport_no'] }}, Nationality
    ...
    @else
        ...
        {{ ($profile['sex'] == 'M') ? 'His' : 'Her' }} course will be completed
        by {{ \Carbon\Carbon::parse(str_replace('/', '-',$profile['idcard_expire'] ))->format('d F Y') }} (probably).
...
<p style="margin-top: 0.1in;">To the best of my knowledge {{ $profile['name'] }} is not involved or working with any
    ...
    involved in any criminal activity. {{ ($profile['sex'] == 'M') ? 'He' : 'She' }} is a full time regular student.</p>
...
<div style="margin-top: 0.3in;">
    <p>{{ $profile['signature_focal_point']['name'] }}</p>
    <p>{{ $profile['signature_focal_point']['position'] }}</p>
    <p>{{ $profile['signature_focal_point']['uni'] }}</p>
    <p>E-mail : {{ $profile['signature_focal_point']['email'] }}</p>
    <p>Cell: {{ $profile['signature_focal_point']['cell'] }}</p>
</div>
