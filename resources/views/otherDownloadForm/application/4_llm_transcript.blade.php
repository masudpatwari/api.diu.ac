...
<table class="header_table">
    ...
    <tr>
        <td colspan="3"
            style="font-size: 20px;text-align: center;font-weight: bold;">LL.M. Examination
            - {{ \Carbon\Carbon::now()->format('Y') }}</td>
    </tr>
        ...
        @foreach ($transcript_data as $transcript_data_key => $transcript_data_value)
            ...
            <tr>
                @if($transcript_data_key == 0)
                    <th style="width: 14%">Course <br> No.</th>
                    <th>Course Title</th>
                    <th style="width: 10%">Credit Hours</th>
                    <th style="width: 10%">Grade Earned</th>
            ...
            @if (is_array($transcript_data_value['allocated_courses']))
                @foreach($transcript_data_value['allocated_courses'] as $allocateSubject)
                    <tr>
                        <td class="tc"><b>{{ $allocateSubject['code'] }}</b></td>
...
@if(!empty($transcript_result))
    <table class="summary_table">
        <tr>
            <th style="text-align: left;border-right: 1px solid #FFFFFF;font-size: 14px">Total Credit Required
            ...
            @endif
            <th style="border-right: 1px solid #FFFFFF;font-size: 14px">Credit Earned
                : {{ $transcript_result['total_credit_earned'] }}</th>
            <th style="border-right: 1px solid #FFFFFF;font-size: 14px">Average Grade
                : {{ $transcript_result['grade_letter'] }}</th>
            <th style="text-align: right;font-size: 14px">CGPA
                : {{ sprintf('%0.2f', $transcript_result['cgpa']) }}</th>
        </tr>
        <tr>
            <th style="text-align: left;border-left: 1px solid #FFFFFF;border-right: 1px solid #FFFFFF;border-bottom: 2px solid #FFFFFF;font-size: 14px">
                Result
                Published
